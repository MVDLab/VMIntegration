...
def main():
    ...
    for f in search.files(DATA_PATH, []):  # a_task or all_tasks_from_one_session
        ...
        # save times
        detected = hmpldat.file.detected.multiindex(df).stack().to_xarray()

        vr = hmpldat.file.dflow.multiindex(df).stack()
        ...
        mocap = hmpldat.file.cortex.multiindex(df).stack().to_xarray()
        print(mocap)
