...
def objective(x, *args):
    ...
    return np.sum([abs_diff_lfc, abs_diff_tfc, sq_distance], axis=0)  # row-wise sum
...
def main():
    ...
    for f in search.files(DATA_PATH, []):  # a_task or all_tasks_from_one_session
        ...
        df = df.assign(
            fnum=hmpldat.file.rawetg.frame_number(df)
        )
        print(df[["Video Time [h:m:s:ms]", "fnum"]])
        ...
        fnum = df[["fnum"]]
        fnum.columns = pd.MultiIndex.from_product([fnum.columns, ['']])
