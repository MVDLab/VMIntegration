...
def main():
    ...
    truth_df = truth_df.assign(
        u=(truth_df.right + truth_df.left) / 2,
        v=(truth_df.bottom + truth_df.top) / 2,
        h=truth_df.bottom - truth_df.top,
        w=truth_df.right - truth_df.left
    )

    detected_df = detected_df.assign(
        u=(detected_df.right + detected_df.left) / 2,
        v=(detected_df.bottom + detected_df.top) / 2,
        h=detected_df.bottom - detected_df.top,
        w=detected_df.right - detected_df.left
    )

    detected_df.to_csv('detected_w_center.csv')
    ...
    for fn in filenames:
        ...
        for i, true_bb in truth.iterrows():
            ...
            for j, detected_bb in detected[detected['object'] == true_bb['object']].iterrows():
                ...
                potential_matches.append((j, iou, detected_bb.u, detected_bb.v, detected_bb.w, detected_bb.h))
      
            # sort by iou
            potential_matches.sort(key= lambda x: x[1])

            # if true_bb['filename'] == "1007.jpg":
            #     print(f"true_bb #{i}, {true_bb['object']}, {true_bb['filename']}")
            ...
            if len(potential_matches) >= 1:
                ...
                matched_detections[fn][i] = {"detected_bb": potential_matches[-1][0], "iou": potential_matches[-1][1], "true_object": true_bb['object'], "true_u": true_bb.u, "true_v": true_bb.v, "detect_u": potential_matches[-1][2], 'detect_v': potential_matches[-1][3], "true_w": true_bb.w, 'true_h': true_bb.h, "detect_w": potential_matches[-1][4], 'detect_h': potential_matches[-1][5], }
            else:
                matched_detections[fn][i] = {"detected_bb": np.NaN, "iou": 0.0, "true_object": true_bb['object'], "true_u": true_bb.u, "true_v": true_bb.v, "true_w": true_bb.w, 'true_h': true_bb.h, "detect_u": np.NaN, 'detect_v': np.NaN, "detect_w": np.NaN, 'detect_h': np.NaN }
    ...
    df = df.assign(
        t_width = df['true_w'] > df['true_h'], 
        d_width = df['detect_w'] > df['detect_h']
    )
    ...
    for _, row in df.iterrows():
        # print(row)
        if row["t_width"]:
            t_ratio.append(row["true_h"] / row["true_w"])
        else: 
            t_ratio.append(row["true_w"] / row["true_h"])

        if row["d_width"]:
            d_ratio.append(row["detect_h"] / row["detect_w"])
        else: 
            d_ratio.append(row["detect_w"] / row["detect_h"]) 

    ratios = pd.DataFrame([t_ratio, d_ratio], index=['truth_ratio', 'detected_ratio'], columns=df.index).T
    print(ratios)
    # print(df)
    ...
    df = df.assign(
        cen2cen=((df.true_u - df.detect_u)**2 + (df.true_v - df.detect_v)**2)**0.5,
        uErr=df.true_u - df.detect_u,
        vErr=df.true_v - df.detect_v
    )

    df = df[df["detected_ratio"] > 0.95]
    print(df[['cen2cen', 'uErr', 'vErr']].describe().to_markdown())

    # print()
    # print(f"uErr: {np.mean(df['uErr'])}")
    # print(f"vErr: {np.mean(df['vErr'])}")
